...
class ResNetTrainer:
    ...
    def _build_model(self):
        ...
        base_model.trainable = False  # Set to True for fine-tuning if needed
    ...
    def process_and_train(self, zip_bytes):
        ...
        logger.info(f"[{self.project}] ResNet model trained and saved to memory (compressed).")
    ...
    def load_local(model_id):
        ...
        if not log:
            raise ValueError("Model ID not found in database")

        trainer = ResNetTrainer(
            project_name=log.project_name,
            num_classes=len(log.class_names.split(",")),
            class_names=log.class_names.split(",")
        )

        decompressed = gzip.decompress(log.model_blob)
        with tempfile.NamedTemporaryFile(suffix=".h5", delete=False) as tmp:
            tmp.write(decompressed)
            tmp.flush()
            trainer.model = tf.keras.models.load_model(tmp.name)
        os.unlink(tmp.name)

        trainer.model_blob = log.model_blob
        return trainer
